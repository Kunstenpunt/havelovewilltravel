...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        venues_without_org = Venue.objects.exclude(raw_venue__icontains="|none|none|").exclude(raw_venue__istartswith="unknown").filter(organisation__isnull=True).filter(non_assignable=False)#.filter(pk__gt=100000)
