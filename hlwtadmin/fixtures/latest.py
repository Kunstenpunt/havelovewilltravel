...
for location in latest[["stad_clean", "land_clean"]].itertuples():
    ...
    if not isnull(city) and not isnull(country) and (city, country) not in location_pk:
        ...
        data = {
            "pk": i,
            "model": "hlwtadmin.Address",
            "fields": {
                "location": i
            }
        }
        address_pk[(city, country)] = i
...
for row in latest[["venue_clean", "stad_clean", "land_clean"]].itertuples():
    ...
    if not isnull(venue) and not isnull(city) and not isnull(country) and (venue, city, country) not in organity_pk:
        address = address_pk[(city, country)]

        data = {
            "pk": i,
            "model": "hlwtadmin.Organity",
            "fields": {
                "name": venue,
                "address": address,
                "organity_type": [1]
...
gigfinders = ["songkick", "bandsintown", "setlist", "facebook", "datakunstenbe", "podiumfestivalinfo", "manual"]
for i, gf in enumerate(gigfinders):
    data = {
        "pk": i + 1,
        "model": "hlwtadmin.Gigfinder",
        "fields": {
            "name": gf
        }
    }
...
for i, gf in enumerate(concertorganitytype):
    data = {
        "pk": i + 1,
        "model": "hlwtadmin.RelationConcertOrganityType",
        "fields": {
            "name": gf
        }
    }
