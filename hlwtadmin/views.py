...
class LocationAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        if self.q:
            ...
            qs = qs.filter(city__unaccent__like=self.q)
        return qs
...
class LocationListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['num_locations'] = Location.objects.filter(city__unaccent__like=context['filtersearch']).filter(country__name=context['filter']).count()
    ...
    def get_queryset(self):
        ...
        if filter_search and filter_val:
            return Location.objects.filter(city__unaccent__like=filter_search).filter(country__name=filter_val).annotate(num_organisations=Count('organisation')).order_by(order)
        ...
        if filter_search and not filter_val:
            return Location.objects.filter(city__unaccent__like=filter_search).annotate(num_organisations=Count('organisation')).order_by(order)
